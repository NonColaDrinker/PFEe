...
<html lang="fr">
  ...
  <body style="background-color: #f2f2f2;">
    ...
    <h2 align="left" style="padding-left:5vh">Commentaires</h2>
    <div  class="container" align="center">
    <form action="./supprimer.php?type=com" method="post" id="asupprimer" onsubmit="return confirmer()">
        <div class="row">
          <div class="col-25">
          </div>
          <div class="col-75">
          <table style="font-family: arial, sans-serif; border-collapse: collapse; width: 100%;">
            <?php           
                        ...
                        <th>ID de l'utilisateur qui a reçu le commentaire</th>
                        <th>Note donnée</th>
                        <th>Supprimer</th>
                        </tr>"; 
            $lignes=mysqli_query($conn,"select * from commentaire ");
            while($row =mysqli_fetch_assoc($lignes) ){
                $note=$row['valeur'];
                $etoiles="";
                for ($i=0;$i<$note;$i++){
                    $etoiles=$etoiles."<img src='assets/img/filled-star.svg'>";
                  }
                echo "<tr><td>" . htmlspecialchars($row['comid']) . "</td><td>" . htmlspecialchars($row['commentateur']) . "</td><td>".htmlspecialchars($row['comment'])."</td><td>".$row['user']."</td><td>".$etoiles."</td><td>"." <input type=\"checkbox\" class=\"supprimer-checkbox\" data-id=\"".intval($row['comid'])."\"></td></tr>";
